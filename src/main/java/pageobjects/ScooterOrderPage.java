...
public class ScooterOrderPage {
    ...
    private final By rentalDurationChoice = xpath("//div[@class='Dropdown-menu']/div[4]"); //Выбор срока аренды (из выпадающего списка)
    private final By scooterColour = xpath("//div[@class='Order_Checkboxes__3lWSI']/label[2]"); //Цвет самоката
    ...
    public void findCheckAndClickOrderButton(By buttonAddress) {
        new WebDriverWait(driver, 5).until(ExpectedConditions.visibilityOfElementLocated(buttonAddress));
        ...
        new WebDriverWait(driver, 5).until(ExpectedConditions.elementToBeClickable(buttonAddress));
